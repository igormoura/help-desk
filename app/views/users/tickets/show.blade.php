				...
				<div class="panel-body">
					<table class="table table-striped newTickets" data-alert="{{ isset($alert) ? $alert : false }}" data-all="{{ isset($newTickets) ? $newTickets : false }}">
						<tbody>
							...
							<tr>	
								<td>{{ trans('translate.attachment_download') }}</td>
								@if(!empty($ticket->attachment))
								<td>								
									@foreach($ticket->attachment as $key => $file)
									<a style="color: #0000EE;" href="{{ asset('public/attachments')}}/{{$file }}" download>{{ $file }}</a><br />
									@endforeach
								</td>
								@else
									<td><a style="color: #0000EE;" href="{{ asset('public/attachments')}}/{{$ticket->attachment}}" download>{{ $ticket->attachment }}</a></td>
								@endif
							</tr>
							</tr>
						<tbody>
